...
public class LogAspect {
	...
	@Before("execution(* com.scaleupindia.controller.*.*(..))")
	public void logBeforeAdvice(JoinPoint joinPoint) {
		logger.info("Entered {} with arguments as {}", joinPoint.getSignature(), joinPoint.getArgs());
	}

	@AfterReturning(pointcut = "execution(* com.scaleupindia.controller.*.*(..))", returning = "result")
	...
	@AfterThrowing(pointcut = "execution(* com.scaleupindia.controller.*.*(..))", throwing = "exception")
